...
const quickActions = [
    { label: 'Record Payment', emoji: 'ðŸ’³', href: '/dashboard/payments', bg: '#f0fdf4', color: '#15803d', ring: '#bbf7d0' },
    { label: 'Mark Paid', emoji: 'âœ…', href: '/dashboard/billing', bg: '#eff6ff', color: '#1d4ed8', ring: '#bfdbfe' },
    { label: 'Statement', emoji: 'ðŸ“„', href: '/dashboard/reports', bg: '#f5f3ff', color: '#6d28d9', ring: '#ddd6fe' },
    { label: 'M-Pesa', emoji: 'ðŸ“±', href: '/dashboard/payments', bg: '#f0fdfa', color: '#0f766e', ring: '#99f6e4' },
    { label: 'Send Bulk SMS', emoji: 'ðŸ’¬', href: '/dashboard/sms', bg: '#fdf4ff', color: '#7e22ce', ring: '#e9d5ff' },
    { label: 'Add Tenant', emoji: 'âž•', href: '/dashboard/tenants', bg: '#fdf2f8', color: '#9d174d', ring: '#fbcfe8' },
];
    ...
    if (collectionRate < 50) {
        return (
            <div className="rounded-2xl px-5 py-4 flex items-center gap-4"
                ...
                <div>
                    <p className="font-bold text-red-800 text-sm">Low collection rate â€” immediate action required</p>
                    <p className="text-red-600 text-xs mt-0.5">{unpaidCount} tenant{unpaidCount !== 1 ? 's' : ''} overdue Â· {fmt(totalOwed)} total owed Â· Only {collectionRate}% collected</p>
                </div>
    ...
    return (
        <div className="rounded-2xl px-5 py-4 flex items-center gap-4"
            ...
            <div>
                ...
                <p className="text-amber-600 text-xs mt-0.5">{fmt(totalArrears)} in arrears Â· {fmt(totalOwed)} total owed Â· {collectionRate}% collected</p>
            </div>
...
export default function DashboardPage() {
    ...
    // -- KPI Cards --
    const kpiCards = [
        { label: 'Total Tenants', value: stats?.activeTenants || 0, emoji: 'ðŸ‘¤', bg: '#eef2ff', border: '#818cf8', sub: `${occupancyRate}% occupancy`, href: '/dashboard/tenants' },
        { label: 'ðŸ†• New Today', value: tenantsNewToday, emoji: 'ðŸŽ‰', bg: '#f0fdf4', border: '#34d399', sub: tenantsNewToday > 0 ? 'Moved in today!' : 'No new tenants', pulse: tenantsNewToday > 0, href: '/dashboard/tenants' },
        { label: 'Occupied Units', value: `${stats?.activeTenants || 0} / ${stats?.totalUnits || 0}`, emoji: 'ðŸšª', bg: '#ecfdf5', border: '#6ee7b7', sub: `${(stats?.totalUnits || 0) - (stats?.activeTenants || 0)} vacant`, href: '/dashboard/units' },
        { label: 'This Month Collected', value: fmt(stats?.monthlyCollected), emoji: 'ðŸ’µ', bg: '#ecfdf5', border: '#34d399', sub: 'Cash + M-Pesa', trend: momChange, href: '/dashboard/payments' },
        { label: 'This Month Billed', value: fmt(stats?.monthlyBilled), emoji: 'ðŸ§¾', bg: '#faf5ff', border: '#a78bfa', sub: 'Total invoiced', href: '/dashboard/billing' },
        { label: 'Paid Today', value: fmt(paidTodayAmount), emoji: 'âœ…', bg: '#f0fdf4', border: '#4ade80', sub: `${paidTodayCount} payment${paidTodayCount !== 1 ? 's' : ''} today`, pulse: paidTodayCount > 0, href: '/dashboard/payments' },
        { label: 'Total Arrears', value: fmt(totalArrearsFromCalc), emoji: 'â°', bg: '#fef2f2', border: '#f87171', pulse: totalArrearsFromCalc > 0, sub: `${unpaidRentData.length} tenants`, href: '/dashboard/unpaid' },
        { label: 'Total Penalty', value: fmt(totalPenaltiesFromCalc), emoji: 'ðŸ’¢', bg: '#fffbeb', border: '#fbbf24', sub: 'Late fees', href: '/dashboard/unpaid' },
        { label: 'Collection Rate', value: `${collRate}%`, emoji: collRate >= 80 ? 'ðŸŒŸ' : collRate >= 50 ? 'ðŸ“ˆ' : 'ðŸ“‰', bg: collRate >= 80 ? '#ecfdf5' : collRate >= 50 ? '#fffbeb' : '#fef2f2', border: collRate >= 80 ? '#34d399' : collRate >= 50 ? '#fbbf24' : '#f87171', sub: collRate >= 80 ? 'Excellent' : collRate >= 50 ? 'Needs attention' : 'Critical', pulse: collRate < 50 && totalArrearsFromCalc > 0, href: '/dashboard/reports' },
        { label: 'Total Owed', value: fmt(totalOwedFromCalc), emoji: 'ðŸ’°', bg: '#fff7ed', border: '#fb923c', sub: 'Incl. penalties', pulse: totalOwedFromCalc > 0, href: '/dashboard/unpaid' },
    ];
